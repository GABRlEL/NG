...
def load_index_spec(path: Path) -> IndexSpec:
    lines = [clean_index_line(line) for line in path.read_text(encoding="utf-8").splitlines()]
    expected_names = {line for line in lines if line}
    return IndexSpec(
        path=path,
...
def discover_index_specs(index_root: Path) -> list[IndexSpec]:
    return [load_index_spec(path) for path in sorted(index_root.glob("*.txt"))]
...
def choose_actual_name_view(index_spec: IndexSpec, directory: Path, direct_names: set[str] | None = None) -> tuple[set[str], str]:
    ...
    best_mode = "direct-children"
    best_names = direct_names
...
def validate_single_directory(index_spec: IndexSpec, directory: Path) -> ValidationResult:
    ...
    actual_names, comparison_mode = choose_actual_name_view(index_spec, directory, direct_names=child_names)
...
def validate_tree(index_specs: list[IndexSpec], export_root: Path) -> list[ValidationResult]:
    ...
    for index_spec in index_specs:
        ...
        actual_names, comparison_mode = choose_actual_name_view(index_spec, matched_dir.path, direct_names=matched_dir.child_names)
        missing_names, unexpected_names = validate_names(index_spec.expected_names, actual_names)
        status = STATUS_OK if not missing_names and not unexpected_names else STATUS_ISSUES
        results.append(
            ValidationResult(
...
def print_summary(results: list[ValidationResult], strict_unexpected: bool) -> None:
...
def main() -> int:
    ...
    if args.index:
        ...
        index_specs = [load_index_spec(index_path)]
        results = [validate_single_directory(index_specs[0], export_path)]
    else:
        index_specs = discover_index_specs(index_root)
        results = validate_tree(index_specs, export_path)

    for result in results:
        print_result(result, verbose=args.verbose, show_ok=args.show_ok)
    print_summary(results, strict_unexpected=args.strict_unexpected)

    if args.json_out:
        payload = {
            "tool": "index_validate.py",
