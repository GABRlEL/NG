...
`index_validate.py` checks extracted output against the cleaned index tables in [Index/List](c:\Users\Gabriel\Downloads\KPD\Index\List).
...
```powershell
python index_validate.py verify_v2_dance_raw --index DATAPACK.KPD__pkdata__dance.txt
```

Validate one extracted folder against one full path index file:

```powershell
python index_validate.py review_battleevent_extract --index Index\List\DATAPACK.KPD__pkdata__battleevent__00_01_004_01_p00pack.txt
```

Validate an export tree against the whole index directory:

```powershell
python index_validate.py path\to\export_root
```
...
```powershell
python index_validate.py verify_v2_dance_raw --index DATAPACK.KPD__pkdata__dance.txt --json-out dance_index_report.json --verbose
```
